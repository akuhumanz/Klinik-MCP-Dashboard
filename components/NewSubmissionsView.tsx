...
interface NewSubmissionsViewProps {
  indData: Participant[];
  corpData: Participant[];
  onProcessAll: () => void;
  onEdit: (participant: Participant) => void;
}

export const NewSubmissionsView: React.FC<NewSubmissionsViewProps> = ({ indData, corpData, onProcessAll, onEdit }) => {
  const [activeTab, setActiveTab] = useState<'perorangan' | 'perusahaan'>('perorangan');
  ...
  const currentData = activeTab === 'perorangan' ? indData : corpData;
  ...
  const processedData = useMemo(() => {
    let result = currentData.filter(item => {
    ...
    return result;
  }, [currentData, sortKey, sortOrder, selectedResult]);
  ...
  return (
    <div className="h-full flex flex-col animate-in fade-in duration-500 overflow-hidden bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 z-20 shrink-0 shadow-sm">
        <div className="p-8 pb-0">
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 mb-1">Pengajuan Baru</h1>
              <p className="text-gray-500 text-sm font-medium">Kelola data pengajuan yang memerlukan verifikasi lanjutan.</p>
            </div>
            <div className="flex items-center gap-3">
              <button 
                onClick={onProcessAll} 
                className="px-8 py-2 bg-blue-600 text-white rounded-lg text-sm font-bold shadow-md hover:bg-blue-700 transition-all active:scale-95"
              >
                Proses Semua Data
              </button>
            </div>
          ...
          <div className="flex gap-8">
            <button 
              onClick={() => { setActiveTab('perorangan'); setSelectedIds([]); setCurrentPage(1); setSelectedResult('Semua Hasil'); }}
              className={`pb-4 text-sm transition-all flex items-center gap-2 ${activeTab === 'perorangan' ? 'border-b-2 border-blue-600 text-blue-600 font-bold' : 'text-gray-500 font-medium hover:text-gray-700'}`}
            >
              Perorangan
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${activeTab === 'perorangan' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>{indData.length}</span>
            </button>
            <button 
              onClick={() => { setActiveTab('perusahaan'); setSelectedIds([]); setCurrentPage(1); setSelectedResult('Semua Hasil'); }}
              className={`pb-4 text-sm transition-all flex items-center gap-2 ${activeTab === 'perusahaan' ? 'border-b-2 border-blue-600 text-blue-600 font-bold' : 'text-gray-500 font-medium hover:text-gray-700'}`}
            >
              Perusahaan
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${activeTab === 'perusahaan' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>{corpData.length}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {/* Toolbar & Table */}
      <div className="flex-1 px-8 py-8 overflow-y-auto space-y-6 pb-24">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="relative w-full md:w-96">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
            <input 
              type="text" 
              placeholder="Cari Nama Peserta, No MCU atau User ID" 
              className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-100 shadow-sm"
            />
          </div>

          <div className="flex items-center gap-3">
            <button className="flex items-center gap-2 px-3 py-2.5 bg-white border border-gray-300 rounded-lg text-gray-700 text-sm font-bold hover:bg-gray-50 whitespace-nowrap shadow-sm">
              <Calendar size={16} />
              Tanggal
            </button>
            ...
            <div className="relative" ref={resultDropdownRef}>
              <button 
                ...
                className="flex items-center gap-2 px-3 py-2.5 bg-white border border-gray-300 rounded-lg text-gray-700 text-sm font-bold hover:bg-gray-50 whitespace-nowrap shadow-sm"
              >
                ...
                <div className="absolute left-0 mt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-100 py-1 z-[60] overflow-hidden">
                  {resultOptions.map((option) => (
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 flex items-center justify-between font-semibold"
                    >
                      <span>{option}</span>
            ...
            <thead>
              <tr className="bg-gray-50 text-[10px] font-bold text-gray-400 uppercase tracking-[0.15em]">
                <th className="px-4 py-5 w-14 border-b border-gray-200 text-center">
                  <div className="flex justify-center"><CustomCheckbox checked={isAllSelected} indeterminate={isIndeterminate} onChange={onToggleSelectAll} /></div>
                </th>
                <th 
                  className="px-4 py-5 border-b border-gray-200 cursor-pointer group hover:bg-gray-100/50 transition-colors"
                ...
                return (
                  <tr key={item.id} className={`${isSelected ? 'bg-blue-50/20' : 'hover:bg-gray-50/30'}`}>
                    ...
                    <td className="px-4 py-4">
                      ...
                      <div className="text-[11px] text-gray-500 font-medium">{item.subText}</div>
                    </td>
                    {activeTab === 'perusahaan' && <td className="px-4 py-4 text-sm font-bold uppercase text-gray-700">{item.company || 'Pribadi'}</td>}
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex items-center justify-center w-7 h-7 rounded-md text-[10px] font-bold ${item.gender === 'P' ? 'bg-lime-50 text-lime-600' : 'bg-blue-50 text-blue-600'}`}>
                        {item.gender}
                      </span>
                    </td>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                    <button
                      ...
                      className={`w-9 h-9 flex items-center justify-center rounded-xl text-sm font-medium transition-all ${
                        currentPage === page ? 'bg-blue-600 text-white shadow-md' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 shadow-sm'
                      }`}
                    >
                      {page}
                    </button>
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 bg-white rounded-xl shadow-2xl border border-gray-200 px-6 py-3 flex items-center gap-6 z-50 animate-in slide-in-from-bottom-2 duration-200">
          <span className="text-sm font-bold text-gray-900 whitespace-nowrap">{selectedIds.length} data dipilih</span>
          <div className="h-6 w-px bg-gray-200"></div>
          <div className="flex items-center gap-2">
            <button className="flex items-center gap-2 text-sm font-bold text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors">
              <Trash2 size={16} />
              Hapus Data
            </button>
            <button 
...
const ActionWithTooltip: React.FC<{ icon: React.ReactNode; label: string; isDelete?: boolean; onClick?: () => void }> = ({ icon, label, isDelete, onClick }) => (
  <div className="group relative flex items-center justify-center">
    <button onClick={onClick} className={`text-gray-400 transition-all p-2 rounded-lg hover:bg-blue-50/50 active:scale-90 ${isDelete ? 'hover:text-red-600 hover:bg-red-50/50' : 'hover:text-blue-600'}`}>
      {icon}
    </button>
